...
let cameraTargetUpOffset = 40; // 跟随时目标上抬（米），让飞机显示在中间偏下
...
const BUILDING_SHADER_OPTIMIZED = `
    void fragmentMain(FragmentInput fsInput, inout czm_modelMaterial material) {
        ...
        if (!u_isDark) {
            ...
            float shaderHeight = positionMC.z - baseHeight;

            float phase = fract(czm_frameNumber / 120.0) * 6.2831853;
            float wave = shaderHeight / heightRange + sin(phase) * 0.1;
            material.diffuse *= vec3(wave);

            float sweep = abs(fract(czm_frameNumber / 100.0) - 0.5) * 2.0;
            float normalizedHeight = clamp(shaderHeight / glowRange, 0.0, 1.0);
            float diffValue = step(0.015, abs(normalizedHeight - sweep));
            material.diffuse += material.diffuse * (1.0 - diffValue) * 0.35;
        } else {
...
const MIN_VISIBLE_BUILDING_HEIGHT = 25.0;
...
async function loadBuildings() {
    try {
        ...
        osmBuildings.style = new Cesium.Cesium3DTileStyle({
            show: `\${feature['cesium#estimatedHeight']} >= ${MIN_VISIBLE_BUILDING_HEIGHT}`
        });
...
function updateInfoPanelPosition(position) {
    ...
    if (screenPosition &&
        ...
        const followOffsetY = -50;

        flightInfo.style.left = screenPosition.x + 'px';
        flightInfo.style.top = (screenPosition.y + followOffsetY) + 'px';
...
function updateCameraFollow(position, currentTime) {
    ...
    const cameraTarget = Cesium.Cartesian3.add(
        position,
        Cesium.Cartesian3.multiplyByScalar(targetUp, cameraTargetUpOffset, new Cesium.Cartesian3()),
        new Cesium.Cartesian3()
    );

    const cameraDirection = Cesium.Cartesian3.normalize(
        Cesium.Cartesian3.subtract(cameraTarget, cameraPosition, new Cesium.Cartesian3()),
        new Cesium.Cartesian3()
    );

    const cameraUp = Cesium.Ellipsoid.WGS84.geodeticSurfaceNormal(
        cameraPosition,
        new Cesium.Cartesian3()
    );
