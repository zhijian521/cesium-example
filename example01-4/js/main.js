...
function createSampledPositionProperty(
  routePoints,
  startTime,
  durationSeconds,
) {
...
function getHeadingBetweenPositions(fromPosition, toPosition) {
  ...
  if (
    Math.abs(deltaLon) < Cesium.Math.EPSILON10 &&
    Math.abs(deltaLat) < Cesium.Math.EPSILON10
  ) {
    return 0;
  }
...
function getBankedAttitude(positionProperty, time) {
  ...
  const previousTime = Cesium.JulianDate.addSeconds(
    time,
    -BANKING_CONFIG.sampleWindowSeconds,
    new Cesium.JulianDate(),
  );
  ...
  const headingBefore = previousPosition
    ? getHeadingBetweenPositions(previousPosition, currentPosition)
    : null;
  const headingAfter = nextPosition
    ? getHeadingBetweenPositions(currentPosition, nextPosition)
    : null;
  const heading = headingAfter ?? headingBefore ?? 0;
  const signedTurnDeltaRadians =
  ...
  const turnRatio =
    absTurnDeltaDegrees <= BANKING_CONFIG.minTurnDeltaDegrees
      ? 0
      : Cesium.Math.clamp(
          absTurnDeltaDegrees / BANKING_CONFIG.fullTurnDeltaDegrees,
          0,
          1,
        );
  ...
  return {
    ...
    roll:
      bankDirection *
      easedTurnRatio *
      Cesium.Math.toRadians(BANKING_CONFIG.maxRollDegrees),
  };
}

function createBankedOrientationProperty(positionProperty) {
  const baseOrientationProperty = new Cesium.VelocityOrientationProperty(
    positionProperty,
  );

  return new Cesium.CallbackProperty((time) => {
    const baseOrientation = baseOrientationProperty.getValue(
      time,
      scratchBaseOrientation,
    );
    ...
    const flightAxis = Cesium.Cartesian3.subtract(
      nextPosition,
      previousPosition,
      scratchFlightAxis,
    );
    if (
      Cesium.Cartesian3.magnitudeSquared(flightAxis) < Cesium.Math.EPSILON10
    ) {
      return Cesium.Quaternion.clone(baseOrientation, new Cesium.Quaternion());
    }
    ...
    osmBuildings.tileVisible.addEventListener((tile) => {
      ...
      for (let featureIndex = 0; featureIndex < featureCount; featureIndex++) {
        ...
        feature.show =
          ...
          resolvedHeight >= BUILDING_MIN_VISIBLE_HEIGHT;
      }
    });
...
function createAirplaneAndPath() {
  ...
  // === 航线1: 东方明珠圆角四边形环线 ===
  const roundedRoutePoints = createRoundedRectangleRoute(
    routeOneCenter,
    routeOneCenter.height,
  );
  const positionProperty1 = createSampledPositionProperty(
    roundedRoutePoints,
    startTime,
    duration,
  );
  ...
  airplaneEntity = viewer.entities.add({
    ...
    model: {
      uri: "../public/models/beta-alia/beta_alia_vtol_aircraft.glb",
      scale: 5,
      minimumPixelSize: 80,
      maximumScale: 100,
      runAnimations: false,
...
// 加速螺旋桨动画 — 通过 ModelVisualizer 内部 API 获取 Model 实例
var propellerSpeedDone = false;
function pollPropellerSpeed() {
  if (propellerSpeedDone) return;

  var dataSource =
    viewer.dataSourceDisplay && viewer.dataSourceDisplay.defaultDataSource;
  var visualizers = dataSource && dataSource._visualizers;
  if (!visualizers) return;

  var allDone = true;
  for (var j = 0; j < airplaneEntities.length; j++) {
    var entity = airplaneEntities[j];
    if (entity._propellerDone) continue;

    var ok = false;
    for (var i = 0; i < visualizers.length; i++) {
      var modelState =
        visualizers[i]._modelHash && visualizers[i]._modelHash[entity.id];
      if (
        modelState &&
        modelState.modelPrimitive &&
        modelState.modelPrimitive.ready
      ) {
  ...
  if (allDone) propellerSpeedDone = true;
}
...
function updateInfoPanelPosition(position) {
  ...
  if (
    ...
    const cameraToPlaneDistance = Cesium.Cartesian3.distance(
      viewer.camera.positionWC,
      position,
    );
    const zoomFactor =
      (cameraToPlaneDistance - INFO_PANEL_DISTANCE_MIN) /
      (INFO_PANEL_DISTANCE_MAX - INFO_PANEL_DISTANCE_MIN);
    const clampedZoomFactor = Cesium.Math.clamp(zoomFactor, 0, 1);
    const easedZoomFactor = 1 - Math.pow(1 - clampedZoomFactor, 5);
    const panelOffsetY = Cesium.Math.lerp(
      INFO_PANEL_OFFSET_Y_MAX,
      INFO_PANEL_OFFSET_Y_MIN,
      easedZoomFactor,
    );
...
function updateFlightData(position, currentTime, airplaneName) {
  ...
  document.getElementById("heading").innerText =
    Math.round(Cesium.Math.toDegrees(heading)) + "°";
...
function updateCameraFollow(position, currentTime) {
  const currentAirplane = airplaneEntities[selectedAirplaneIndex];
  const attitude = currentAirplane
    ? getBankedAttitude(currentAirplane.position, currentTime)
    : null;
  ...
  const targetUp = Cesium.Ellipsoid.WGS84.geodeticSurfaceNormal(
    position,
    new Cesium.Cartesian3(),
  );
  const targetOffset = Cesium.Cartesian3.multiplyByScalar(
    targetUp,
    16,
    new Cesium.Cartesian3(),
  );
...
function setupEventListeners() {
  ...
  handler.setInputAction(function (click) {
    ...
    if (Cesium.defined(pickedObject)) {
      const clickedIndex = airplaneEntities.findIndex(
...
function toggleCameraLock() {
  ...
  if (isCameraLocked) {
    ...
    // 添加滚轮缩放监听
    viewer.canvas.addEventListener("wheel", handleCameraZoom, {
      passive: false,
    });
  } else {
    unlockCamera();
